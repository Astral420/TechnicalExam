...
<div class="space-y-6 max-w-xl">
    ...
    <div class="flex items-center gap-2 text-[13px] text-[#6B5D45]">
        ...
        <span>/</span>
        <span class="text-[#3D3428] font-medium">New Entry</span>
    </div>

    <!-- Form Card -->
    <div class="bg-[#FFFDF7] border border-[#D4C5A9] rounded-lg p-6 sm:p-8">
        <h1 class="text-section mb-6">Create New Book</h1>
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           class="w-full bg-[#FFFDF7] border @error('title') border-[#B44D3B] @else border-[#D4C5A9] @enderror text-[#3D3428] placeholder-[#9C8E76] rounded-md px-3 py-2.5 text-[14px] focus:outline-none focus:border-[#C9A84C] focus:ring-2 focus:ring-[#C9A84C]/25" />
                    @error('title')
                ...
                <div>
                    ...
                    <select name="author_id"
                        ...
                        @foreach($authors as $author)
                            <option value="{{ $author->id }}" @selected(old('author_id', request('author_id')) == $author->id)>
                                {{ $author->name }}
                            </option>
                        @endforeach
                    </select>
